...
const CreateContainer = () => {
  ...
  const uploadImage = (e) => {
    ...
    uploadTask.on('state_changed', (snapshot) => {
      ...
      console.log(error);
      setFields(true);
      setMsg('Erro durante o upload: Tente novamente');
  ...
  const saveDetails = () => {
    ...
    try {
      if((!title || !imageAsset || !price || !category)){
        setFields(true);
        setMsg('Alguns campos não podem ficar vazios');
        setAlertStatus('danger');
        setTimeout(() => {
          setFields(false)
          setIsLoading(false)
      ...
      <div className='w-[90%] md:w-[75%] border border-gray-300 rounded-lg p-4 flex flex-col items-center justify-center gap-4'>
        {
          ...
          <div className='w-full '>
            <select value={category} onChange={(e) => setCategory(e.target.value)} className="outline-none w-full text-base border-b-2 border-gray-200 p-2 rounded-md cursor-pointer">
              <option value="other" className='bg-white'>Selecione a categoria</option>
              {categories && 
                categories.map(item => (
                    ...
                    {!imageAsset ? (<>
                      <label className='w-full h-full flex flex-col items-center justify-center cursor-pointer'>
                        <div className='w-full gap-2 h-full flex flex-col items-center justify-center'>
                          ...
                          <p className='text-gray-500 hover:text-gray-700'>Clique para fazer upload</p>
                        </div>
          ...
          <div className='flex items-center w-full'>
            <button type='button' className='ml-0 md:ml-auto w-full md:w-auto border-none outline-none bg-emerald-500 px-12 py-2 rounded-lg text-lg text-white font-semibold' onClick={saveDetails}>Adicionar</button>
          </div>
      </div>
    </div>
  )
